...
| 背景资料源表 | 已获取，分级可用 | `spike/out_delivery/background_context_sources.csv` | 背景图表、故事旁白、引用线索 | 24 条资料，20 条高可信，3 条中可信文化线索，1 条人均禽类消费缺口记录 |
| 广州行政区边界 | 已获取 | `spike/data/gz_boundary.json` | 渗透率分区专题图底图 | 若 Windows 端没有该文件，可从 `spike/data/` 复制到制图目录 |
| 背景资料缺口检索记录 | 已更新 | `spike/out_delivery/background_gap_review_2026-07-01.md` | 记录已补资料、仍缺资料和使用限制 | 正式展示前按该记录复核中可信来源 |
| 广东人均禽类消费量 | 待获取 | `background_gap_review_2026-07-01.md` | 只用于宏观背景，不参与 POI 渗透率 | 未找到公开官方网页数值；暂用广东禽肉产量、禽肉 CPI、食品烟酒支出替代 |
| 清远鸡、白切鸡、粤菜消费公开材料 | 已补中可信线索 | `background_context_sources.csv` | 用于故事升华和文字佐证 | 不能替代 POI 数据；正式答辩前最好核对原始政府公告 |
| 广东地方鸡代表点位 | 可做示意，待权威复核 | `background_context_sources.csv` | 可做故事开头或辅助定位图 | 用清远麻鸡、杏花鸡、惠阳胡须鸡、阳山鸡等代表性品种的原产地/保护地，不伪造具体店铺或养殖场点 |
...
| 注意事项 | 地方鸡文化材料目前是中可信线索；不要把宏观消费数据直接解释成广州 POI 分布原因 |

### 图 8：广东地方鸡文化代表点图

| 项目 | 设置 |
|---|---|
| 使用数据 | `background_context_sources.csv` + `background_gap_review_2026-07-01.md` |
| 推荐口径 | 清远麻鸡/清远鸡、杏花鸡、惠阳胡须鸡、阳山鸡等代表性地方鸡种的原产地或地理标志区域 |
| 呈现效果 | 作为故事开头或结尾的文化来源图，不作为广州 POI 统计图 |
| 注意事项 | 当前只能做示意点或区域标注；正式答辩前需核对农业农村部品种名录和地理标志公告原文 |
...
| 筛选建议 | 正式统计图表只使用 `confidence=高` 的行；文化叙事可使用 `confidence=中`，但需在资料页标明仍待原始公告复核 |
